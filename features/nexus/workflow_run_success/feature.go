...
var Feature = harness.Feature{
	...
	CheckHistory: func(ctx context.Context, runner *harness.Runner, run client.WorkflowRun) error {
		...
		callerLink := findWorkflowEventLink(handlerStarted.GetLinks(), enumspb.EVENT_TYPE_NEXUS_OPERATION_SCHEDULED)
		if callerLink == nil {
			return fmt.Errorf("handler WorkflowExecutionStarted is missing a link to the caller NexusOperationScheduled event")
		}
		if callerLink.GetWorkflowId() != run.GetID() || callerLink.GetRunId() != run.GetRunID() {
			return fmt.Errorf("handler link references %s/%s, expected caller %s/%s",
